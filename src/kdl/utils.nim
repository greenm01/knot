...
const
  ...
  escapeTable* = {
    ...
    'f': "\u000C", # Form Feed
    's': "\u0020", # Space
    # Note: Forward slash (/) is NOT escapable in KDL 2.0
  }.toTable
...
proc quoted*(x: string): string =
  ...
  while i < x.len:
    ...
    for k, v in escapeTable:
      # Don't escape space (KDL 2.0 spec - space escape is optional for readability)
      if k == 's':
        continue
...
proc needsQuoting*(s: string): bool =
  ## Returns true if the string needs to be quoted as a KDL identifier
  ## KDL 2.0 spec: bare identifiers cannot:
  ## - Be empty
  ## - Look like numbers (start with digit, +/- digit, or . digit)
  ## - Be disallowed keywords without # prefix (inf, -inf, nan)
  ## - Contain special characters or whitespace

  if s.len == 0:
    return true

  # Check for disallowed bare keywords (must use # prefix in KDL 2.0)
  if s in ["inf", "-inf", "nan"]:
    return true

  # Check if starts with numeric pattern
  if s[0] in Digits:
    return true
  if s[0] == '.' and s.len > 1 and s[1] in Digits:
    return true
  if s[0] in {'+', '-'} and s.len > 1 and s[1] in Digits:
    return true

  # Check for special characters that require quoting
  # KDL 2.0 spec: only these characters are disallowed in bare identifiers
  const nonIdenChars = {'\\', '/', '(', ')', '{', '}', ';', '[', ']', '=', '"', '#'}
  for c in s:
    if c in nonIdenChars:
      return true
    ...
    if rune.int <= 0x1F or rune.int == 0x7F:
      return true
